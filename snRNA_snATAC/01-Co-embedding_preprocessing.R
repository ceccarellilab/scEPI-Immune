...
malignant.multiome = malignant.multiome %>% 
  ...
  FindClusters(resolution = 0.1) %>% 
  RunTSNE(dims = 1:13, reduction.name = 'tsne_RNA', reduction.key = 'tsneRNA_')
...
tme.multiome = tme.multiome %>% 
  FindNeighbors(reduction = 'pca', dims = 1:17) %>% 
  FindClusters(resolution = 0.1) %>% 
  RunTSNE(dims = 1:17, reduction.name = 'tsne_RNA', reduction.key = 'tsneRNA_', check_duplicates = FALSE)
